...
//            String url = serverConfig.getUrl() + fileName;
            ...
            // 从原始文件名中提取文件扩展名，例如 ".jpg"
            String extension = originalFilename.substring(originalFilename.lastIndexOf("."));
            // 使用 UUID 生成一个唯一的文件名，并加上提取到的扩展名，避免文件名重复
            String fileName = UUID.randomUUID().toString()+extension;
            // 调用文件存储服务，将文件内容上传到 OSS（对象存储服务），并获取返回的文件访问 URL
            String url = fileStorageService.store(fileName,file.getInputStream());
            // 创建一个 AjaxResult 对象，表示操作结果（成功）
...
//            ajax.put("newFileName", FileUtils.getName(fileName));
            ...
            // 返回 AjaxResult 对象，包含上传结果信息，前端可以通过这个结果获取上传后的文件信息
            return ajax;
        }
        catch (Exception e)
        {
